$('#btn').on('click', function(){
  ...
  if(validateGrid(grid)){
    ...
    alert("Wrong format ..... Please Enter Number In The Box")
  }
})
...
function doTheJob(code, actions){
  ...
  for(i=0; i<3; i++){
    for(j=0; j<3; j++){
      ...
      $('tbody tr').eq(zeroPos[0]).find('td').eq(zeroPos[1]).css('background-color', 'Orange')
  ...
  var iterations = setInterval(function(){
   var loo = loops +1;
    if(loops==actions.length){
      ...
      demo[loops] = "  step " + loo+" Is :"  +  actions[loops]+"<br>"  ;
